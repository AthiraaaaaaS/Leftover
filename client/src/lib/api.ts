...
function setSession(token: string, user: unknown) {
  ...
  localStorage.setItem("leftoverlink_auth_current_user_v1", JSON.stringify(user));
}
...
export const api = {
  ...
  async listTasks(volunteerId: string) {
    return fetchApi<unknown[]>(`/tasks?volunteer_id=${encodeURIComponent(volunteerId)}`);
  },
  ...
  maps: {
    async geocode(address: string) {
      return fetchApi<{
        results: { formatted_address?: string; place_id?: string; location: { lat: number; lng: number } }[];
    ...
    async placesAutocomplete(input: string, sessionToken?: string) {
      ...
      return fetchApi<{
        predictions: { place_id?: string; description?: string; structured_formatting?: unknown }[];
