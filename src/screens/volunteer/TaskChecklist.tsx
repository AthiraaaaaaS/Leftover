...
export default function TaskChecklist() {
  ...
  async function load() {
    if (!taskId) return;
    setTask(await mockApi.getTask(taskId));
  }
        ...
        <CardContent className="p-4 space-y-4">
          <Row
            ...
            onCheckedChange={async (v) =>
              setTask(await mockApi.saveChecklist(task.id, { sealed: !!v }))
            }
          />
          <Row
            ...
            checked={c.labelled}
            onCheckedChange={async (v) =>
              setTask(await mockApi.saveChecklist(task.id, { labelled: !!v }))
            }
          />
          <Row
            ...
            checked={c.noLeak}
            onCheckedChange={async (v) =>
              setTask(await mockApi.saveChecklist(task.id, { noLeak: !!v }))
            }
          />
          <Row
            ...
            checked={c.onTime}
            onCheckedChange={async (v) =>
              setTask(await mockApi.saveChecklist(task.id, { onTime: !!v }))
            }
          />
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                await mockApi.saveChecklist(task.id, {
